...
[p2,h02,stats2] = ranksum(contact_size_per_true_pos, contact_size_per_false_pos);
f5v2 = figure('Position',[100 100 850 800]);
violin_data = [];
...
vp = violinplot(violin_data(:,2), round(violin_data(:,1)), 'ViolinColor', [200,200,200]/255);
for i = 1:2
...
set(gcf,'color','w');
xlabel('Synapse (1) , Non-synapse (2)');
ylabel('CF Contact Area (\mum^2)');
set(gca,'FontSize',19);hold on;
...
% normality test
[h1,pn1] = kstest((contact_size_per_true_pos - mean(contact_size_per_true_pos))/std(contact_size_per_true_pos));
[h2,pn2] = kstest((contact_size_per_false_pos - mean(contact_size_per_false_pos))/std(contact_size_per_false_pos));
[h3,pn3] = kstest((contact_size_per_true_pos_in1 - mean(contact_size_per_true_pos_in1))/std(contact_size_per_true_pos_in1));
[h4,pn4] = kstest((contact_size_per_true_pos_in2 - mean(contact_size_per_true_pos_in2))/std(contact_size_per_true_pos_in2));
[h5,pn5] = kstest((contact_size_per_false_pos_in1 - mean(contact_size_per_false_pos_in1))/std(contact_size_per_false_pos_in1));
[h6,pn6] = kstest((contact_size_per_false_pos_in2 - mean(contact_size_per_false_pos_in2))/std(contact_size_per_false_pos_in2));

disp(['kstest p-val=',num2str(pn1)])
disp(['kstest p-val=',num2str(pn2)])
disp(['kstest p-val=',num2str(pn3)])
disp(['kstest p-val=',num2str(pn4)])
disp(['kstest p-val=',num2str(pn5)])
disp(['kstest p-val=',num2str(pn6)])

% since N>30 do t-test
[h7,p7] = ttest2(contact_size_per_true_pos,contact_size_per_false_pos);
[h8,p8] = ttest2(contact_size_per_true_pos_in1,contact_size_per_true_pos_in2);
[h9,p9] = ttest2(contact_size_per_false_pos_in1,contact_size_per_false_pos_in2);
[h10,p10] = ttest2(contact_size_per_true_pos_in1,contact_size_per_false_pos_in1);
[h11,p11] = ttest2(contact_size_per_true_pos_in2,contact_size_per_false_pos_in2);

disp(['t-test p-val=',num2str(p7)])

[p3,h03,stats3] = ranksum(contact_size_per_true_pos_in1, contact_size_per_true_pos_in2);
[p4,h04,stats4] = ranksum(contact_size_per_false_pos_in1, contact_size_per_false_pos_in2);
[p5,~,~] = ranksum(contact_size_per_true_pos_in1, contact_size_per_false_pos_in1);
[p6,~,~] = ranksum(contact_size_per_true_pos_in2, contact_size_per_false_pos_in2);

%save_figure(f5v2, 'fig4k_contact_area_syn_vs_nonsyn', 'svg');
f5v3 = figure('Position',[100 100 850 800]);
violin_data = [];
...
violin_data = [violin_data; repmat(4, numel(contact_size_per_false_pos_in1), 1), contact_size_per_false_pos_in1];
violin_data = [violin_data; repmat(5, numel(contact_size_per_false_pos_in2), 1), contact_size_per_false_pos_in2];
...
%% Script for the pre-computed data
