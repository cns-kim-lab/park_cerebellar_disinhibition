...
for i = 1:ntrial
    ...
    y_in_absent(2,:) = y_in_absent(2,:) + (-1*double_exp(t-2.5, 1, -1, a, b)); % motif 2 PF
...
for i = 1:ntrial
    ...
    y_in_absent(2,:) = y_in_absent(2,:) + (-1*double_exp(t-2.5, 1, -1, a, b)); % motif 2
...
% --------------------------------------------------------------------------------------------------------------------------------    
    ...
    xx = (2.5+abs(min(t2)))/(max(t2)-min(t2))*0.74+0.13;
    annotation('line', [xx,xx], [0.57, 0.55], 'LineWidth', 2, 'Color', [170,80,170]/255);        
    xlim([min(t2),max(t2)]); ylim([-0.3, ylim_magnitude]); 
...
yy_cf_inhib = 0.3*double_exp(tt-4.5, 1, -1, 8, 2);
