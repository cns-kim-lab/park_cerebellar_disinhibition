...
:        PVnull  = .08           (mM)
  ...
  : to eliminate pump, set TotalPump to 0 in hoc
    ...
    Kp = 2.7e-3 (mM)
	cao = 2	(mM)
	diam_fixed = 6.026655100953909 (um)
}
...
PROCEDURE factors() {
	LOCAL r, dr2, dr3, radii0, radii1, radii2, radiiLast

	r = diam_fixed/2                : starts at edge (half diam)
:	printf("diam = %f\n", diam_fixed)
:	dr2 = 0.1  : full thickness of outermost annulus,
	...
	radii1 = r - dr2
	radii2 = r - dr2 - dr3
	radiiLast = r - dr2 - (Nannuli-2)*dr3

	vrat[0] = PI*((radii0*radii0)-(radii1*radii1))
	vrat[1] = PI*((radii1*radii1)-(radii2*radii2))

	frat[0] = 2*r
	frat[1] = 2*PI*radii1/(dr2+(dr3/2))

	vrat[Nannuli-1] = PI*radiiLast*radiiLast
	frat[Nannuli-1] = 2*PI*radiiLast/((dr3/2)+radiiLast)

	FROM i=2 TO Nannuli-2 {
:		printf("%f\n",r - dr2 - (i-1)*dr3)
		vrat[i] = PI*((r - dr2 - (i-1)*dr3)*(r - dr2 - (i-1)*dr3)-(r - dr2 - i*dr3)*(r - dr2 - i*dr3))
		frat[i] = 2*PI*(r - dr2 - (i-1)*dr3)/dr3
	}
:	printf("%f\n",radiiLast)
}
