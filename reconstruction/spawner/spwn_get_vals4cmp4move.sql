delimiter //
drop procedure if exists spwn_get_vals4cmp4cmp //
drop temporary table if exists results //
drop temporary table if exists candidates //

create procedure spwn_get_vals4cmp4cmp(IN cell int)
begin

declare target_task int;
select id into target_task from tasks where progress=2 && status=1 && cell_id=cell limit 1;
...
from(select * from validations where task_id=target_task && status=2 && inspected=0) as cdd  
