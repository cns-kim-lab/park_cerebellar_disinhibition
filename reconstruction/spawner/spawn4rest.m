function rtn_spawn = spawn4rest(test_cell, gthresh, max_add_task,test_task_id,force2)
...
clck=floor(clock);
log_file_name = sprintf('spawn4rest_cell%d_%d-%d-%d',test_cell,clck(1:3));
...
%for migration result log
log_file_name2 = sprintf('mig_result_cell%d_%d-%d-%d',test_cell,clck(1:3));
log_path = [log_dir log_file_name2];
fid2 = fopen(log_path,'a+');

all_task = mysql(h_sql, sprintf('select id from tasks where cell_id=%d', test_cell));
fprintf(fid2, 'number of tasks before: %d\n',length(all_task));

root_task = mysql(h_sql, sprintf('select id from tasks where depth=0 && cell_id=%d',test_cell));
rst_step = mysql(h_sql, sprintf('update tasks set status=0 where id=%d;', root_task));
rtn_step = mysql(h_sql, sprintf(['update consensuses as c join tasks as t ' ...
    'on t.id=c.task_id && t.latest_consensus_version=c.version set c.inspected=0 ' ...
    'where t.id=%d;'], root_task))
        ...
        for i=1:n_exits
            ...
            if rtn_exist==0
                ...
                elseif any( (dup_seg_list{1}(:)==test_cell) .* (dup_seg_list{6}(:)==1) )
                    ...
                    fprintf(fid,'\t(task_id:%d)add new task at vol(id:%d) seeds %d,...\n', ...
                        last_task_id,vol_seg_list{drct}{i}.vol,vol_seg_list{drct}{i}.seg_list(1));
                    fprintf(fid,'this task crash with other cell\n');
                    fprintf('\t(task_id:%d)add new task at vol(id:%d) seeds %d,...\n', ...
                        last_task_id,vol_seg_list{drct}{i}.vol,vol_seg_list{drct}{i}.seg_list(1));
                    fprintf('this task crash with other cell\n');
...
%for migration result
all_task_aft = mysql(h_sql, sprintf('select id from tasks where cell_id=%d', test_cell));
fprintf(fid2, 'number of tasks after: %d\n',length(all_task_aft));

not_stitched = mysql(h_sql, sprintf('select id from tasks where cell_id=%d && status=1', test_cell));
dupped = mysql(h_sql, sprintf('select id from tasks where cell_id=%d && status=2', test_cell));
fprintf(fid2, 'not stitched : %d\n', length(not_stitched));
fprintf(fid2, 'duplicated : %d\n', length(dupped));
fprintf(fid2, 'not sheet tasks :\n');
fprintf(fid2, '%d ',setdiff(all_task_aft,all_task));
fprintf(fid2, '\n');
