...
global mip_level mip_factor scaling_factor global_thresh default_vol_overlap default_vol_size
...
net_priority=lrrtm3_get_net_priority();
...
for search_direction=1:6
    ...
    % get next vol segmentation file name, coordinate and size 
    [next_vols_id, next_vols_path, next_vols_ijk,next_vols_coord,next_vols_size] = ...
        get_next_volume_info(this_vol_ijk,search_direction,net_priority);
    if ~isempty(next_vols_id)
        drct_vols_ijkid(search_direction,1) = {[next_vols_ijk{1} next_vols_id]};
    end
        
    % if no seg file exists at all, return no volume for this direction
    if isempty(next_vols_path)
        status{search_direction}=1; 
        continue;
    end

    for ii=1:numel(next_vols_path)
    
        next_vol_id=next_vols_id(ii);
        next_vol_path=next_vols_path{ii};
        next_vol_seg_path = sprintf(...
            '%s.files/segmentations/segmentation1/%d/volume.uint32_t.raw',next_vol_path,mip_level);
        next_vol_coord=next_vols_coord{ii};
        next_vol_size=next_vols_size{ii};
...
%             idx = ratio>fraction_threshold & num_total_voxels_of_seg_in_overlap>size_threshold;
            ...
            %idx = num_total_voxels_of_seg_in_overlap>size_threshold; 
            ...
            if ~isempty(idx)
                ...
                [sl, ss] = get_seglist_at_thresh(next_vol_path, seg_list_next_vol, global_thresh);
                ...
                %{
                next_vol_seg_list{search_direction}{n_result}.entering_coord
                next_vol_seg_list{search_direction}{n_result}.seg_list
                next_vol_seg_list{search_direction}{n_result}.vol
                next_vols_ijk
                aaa=1;
            %}
            end
...
%%
function [seg_list, seg_size] = get_seglist_at_thresh(vol_path, seed_seg_id, thresh)
