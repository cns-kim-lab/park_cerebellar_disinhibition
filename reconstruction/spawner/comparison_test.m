function rtn_comparison = comparison_test()
...
clck=floor(clock);
log_file_name = sprintf('comparison_%d-%d-%d',clck(1:3));
...
while isdone==false
    ...
    [rtn_cpr, rst_cell] = compare_tasks(task_id, pth, validation_id, user_id, segments);
        ...
        rtn_step = mysql(h_sql,'start transaction;');
        rtn_add = add_comparison_task(task_id, rtn_cpr, rst_cell);
...
fprintf(fid,'\n(task_id:%d) [user %s&%s] Comparing...',task_id,user1,user2);
fprintf('\n(task_id:%d) [user %s&%s] Comparing...',task_id,user1,user2);
...
if (~isempty(seg_list_error1) || ~isempty(seg_list_error2))
    ...
    if (~isempty(seg_list_error1))
        ...
        [~,indx]=sort(size_cc_error1,'descend');
        
        size_tmp = size_cc_error1;
        list_tmp = list_seg_in_cc_error1;
        
        for iii=length(indx):-1:2
            plg = 0;
            for ii=iii-1:-1:1
                if ~isempty(intersect(list_tmp{indx(ii)}, list_tmp{indx(iii)})) && plg==0
                    size_tmp(indx(ii)) = size_tmp(indx(ii)) + size_tmp(indx(iii));
                    size_tmp(indx(iii)) = 0;
                    list_tmp(indx(ii)) = {unique([list_tmp{indx(ii)};list_tmp{indx(iii)}])};
                    list_tmp(indx(iii)) = {setdiff( list_tmp{indx(iii)}, intersect(list_tmp{indx(ii)}, list_tmp{indx(iii)}) )};
                    plg=1;
                end
        ...
        for ind=1:length(indx)
           if size_tmp(indx(ind))~=0 && ~isempty(list_tmp(indx(ind)))
               size_cc_error1(end+1) = size_tmp(indx(ind));
               list_seg_in_cc_error1{end+1} = list_tmp{indx(ind)};
           end
        end
        size_cc_error1 = size_cc_error1';
    ...
    if (~isempty(seg_list_error2))
        ...
        [~,indx]=sort(size_cc_error2,'descend');
        
        size_tmp = size_cc_error2;
        list_tmp = list_seg_in_cc_error2;
        ...
        for ind=1:length(indx)
           if size_tmp(indx(ind))~=0 && ~isempty(list_tmp(indx(ind)))
               size_cc_error2(end+1) = size_tmp(indx(ind));
               list_seg_in_cc_error2{end+1} = list_tmp{indx(ind)};
           end
        end
        size_cc_error2 = size_cc_error2';
...
if (~isempty(seg_list_error1) || ~isempty(seg_list_error2))
    ...
    if (exist('size_cc_error1','var'))
        ...
        for i=1:numel(size_cc_error1)
            ...
            if seg_cc_size > dust_fraction_thresh && seg_cc_voxels > dust_voxel_thresh
                this_cc_seg = list_seg_in_cc_error1{idx_cc_error1_sorted_by_size(i)};
                if ~isempty( intersect(all_cc_seg, this_cc_seg) )
                    continue;
                end
                ...
                rst_cell{2,1} = [rst_cell{2,1} sprintf('%d ',this_cc_seg)];
                all_cc_seg = [all_cc_seg this_cc_seg'];
                i2 = i2+1;
            else
    ...
    if (exist('size_cc_error2','var'))
        ...
        for i=1:numel(size_cc_error2)
            ...
            seg_cc_size = seg_cc_voxels/size_selected_seg_total*100;
            if seg_cc_size > dust_fraction_thresh && seg_cc_voxels > dust_voxel_thresh
                this_cc_seg = list_seg_in_cc_error2{idx_cc_error2_sorted_by_size(i)};
                if ~isempty( intersect(all_cc_seg, this_cc_seg) )
                    continue;
                end
                ...
                rst_cell{3,1} = [rst_cell{3,1} sprintf('%d ',this_cc_seg)];
                all_cc_seg = [all_cc_seg this_cc_seg'];
                i3=i3+1;
            else
                dust_size = dust_size + seg_cc_size;
...
%%
function rtn = add_comparison_task(task_id, rtn_cpr, rst_cell)
global h_sql system_user_id fid
rtn=0;
...
rtn_step = mysql(h_sql, sprintf(['update tasks set progress=3, comparison_group_id=%d where id=%d;'], ...
    group_id,task_id));
...
for i=1:num_seg_group
    ...
    %add consensus when comparison is perfect
    if rtn_cpr==2 && i==1
        %comparison_id = mysql(h_sql, sprintf('select last_insert_id();'));
        ltst_cons_vsn = mysql(h_sql,sprintf(...
            ['select latest_consensus_version from tasks where id=%d'],task_id));
        rtn_add_cons = mysql(h_sql, ... 
            sprintf(['insert into consensuses ' ...
                '(task_id,user_id,comparison_group_id,segments,status,version) '...
                'values(%d,%d, %d,"%s",%d,%d);'], ...
                task_id, system_user_id, group_id, rst_cell{1,1},2,ltst_cons_vsn+1));
        if rtn_add_cons==1
            
            last_id = mysql(h_sql,'select last_insert_id();');
            %{
            fprintf(fid,'(cons_id:%d)insert into consensuses for task_id:%d, comparison_group_id=%d\n', ...
                last_id,task_id, group_id);
            fprintf('(cons_id:%d)insert into consensuses for task_id:%d, comparison_group_id=%d\n', ...
                last_id,task_id, group_id);
%}          
            fprintf(fid,'(task_id:%d) consensused\n',task_id);
            fprintf('(task_id:%d) consensused\n',task_id);
            %set task progress to 5 (consensus)
            %{
            fprintf(fid,'(task_id:%d)set task progress to 5 (consensus) and latest cons version ++\n',task_id);
            fprintf('(task_id:%d)set task progress to 5 (consensus) and latest cons version ++\n',task_id);
%}            
            rtn_step = mysql(h_sql, sprintf(['update tasks set progress=5, ' ...
                'latest_consensus_version=latest_consensus_version+1 where id=%d;'],task_id));
        else
            fprintf(fid,'can not insert into comparisons for task_id:%d, comparison_group_id=%d\n', ...
                task_id, group_id);  
            fprintf('can not insert into comparisons for task_id:%d, comparison_group_id=%d\n', ...
                task_id, group_id); 
            return
        end
    end
