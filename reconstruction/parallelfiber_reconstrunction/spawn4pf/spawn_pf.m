function spawn_pf(cell_id, task_id, recursive)
    if nargin==3 && strcmpi(recursive, 'all')
        cnt_iter = 0;
        task_ids = task_id;
        while 1
            child_ids = spawn_tasks(cell_id, task_ids);
            if isempty(child_ids)
                break                
            end            
            cnt_iter = cnt_iter+1;
            if cnt_iter > 15 %quit condition
                fprintf(' too many new tasks, stop spawn. please retry.\n');
                break
            end
            task_ids = child_ids;
        end
    else
        [~, ~] = spawn_pf_(cell_id, task_id);
    end
end
%%
function child_ids = spawn_tasks(cell_id, task_ids)
    child_ids = [];
    if isempty(task_ids)
        return
    end
    ...
    for iter=1:numel(task_ids)
        [new_ids, stitched_ids] = spawn_pf_(cell_id, task_ids(iter));
        child_ids = [child_ids new_ids stitched_ids];
    end
end

%%

function [new_ids, stitched_ids] = spawn_pf_(test_cell, test_task_id)
    % jwgim: clear previous connection and re-connect 
    mysql('close');    
    global h_sql     
    connsql();
    ...
    stitched_ids = [];
    new_ids = [];
    % jwgim: exception handling (login?)
    user = account();
    if isempty(user)
    ...
    test_mode = 2;
    force2 = 1;
    global global_thresh system_user_id tstcell
    tstcell = test_cell;    
    global_thresh = gthresh;
    
    % jwgim: exception handling (consider only my cell)
    write_log(sprintf(' [ spawn by manual ] cell %d, task %d.', test_cell, test_task_id), 1);
    verified_mine = mysql(h_sql, sprintf('SELECT c.id FROM cells c INNER JOIN cell_metadata m ON m.id=c.meta_id WHERE c.id=%d AND m.notes LIKE ''%s%%'';', test_cell, user));
    if isempty(verified_mine)
        write_log(' can''t spawn(not my cell).', 1);
        mysql(h_sql,'close');
        return
    end
    %----------------------------------------------------------------------------------------------------------------
...
while isdone==false && count_new<max_add_task
    ...
    if isempty(cons_id)
        isdone = true;
        write_log('       no more consensuses to spawn', 1);
        continue
    end
    ...
    write_log(sprintf('     %d: spawning.. ', task_id), 1);
    ...
    if rtn_exist~=0
        ...
        for i=1:numel(dup_seg_list{1})
            write_log(sprintf('[%d] (cell_id:%d) duplication', dup_seg_list{3}(i),dup_seg_list{1}(i)), 1);
        ...
        for drtsk=1:length(duple_recoded_task)
            ...
            if ~isempty(dup_seg_list_mc)
                if ismember(task_id, dup_seg_list_mc{3}) && status_mc~=1 && status_mc~=3
                    ...
                    write_log(sprintf('       %d (cell_id:%d) duplication from related task',task_id_mc, cell_id_mc), 1);
    ...
    for drct=1:6
        if ~isempty(drct_vols_ijkid{drct})
            ...
            for ii=1:numel(child_id)
                ...
                if all(doprune)
                    ...
                    write_log(sprintf('       %d (%s) pruned',child_id(ii),drct_str(drct)), 1);
                end
            end
        ...
        for i=1:n_exits
            ...
            if rtn_exist==0
                ...
                write_log(sprintf('       %d (%s) spawned',last_id,drct_str(drct)), 1);
                
                count_new = count_new+1;
                count_all = count_all+1;    
                new_ids = [new_ids last_id];
                
                %modify by jwgim 180910 (notes field update)
                rtn = mysql(h_sql, sprintf('update tasks set notes="pass agg" where id=%d;', last_id));
            else
...
%                     dup_task = dup_seg_list{3}(logical((dup_seg_list{1}(:)==test_cell) .* (dup_seg_list{6}(:)~=1)));
                    ...
                    for iii=1:length(dup_task)
                        write_log(sprintf('       %d (%s) already exists',dup_task(iii),drct_str(drct)), 1);
                ...
                elseif any( dup_seg_list{6}(:)==1 )
                    ...
                    for chid = 1:numel(stashed_child_task_id)
                        ...
                        write_log(sprintf('       %d (%s) stitched',stashed_child_task_id(chid),drct_str(drct)), 1);
                        ...
                        %modify by jwgim 180910 (notes field update)
                        rtn = mysql(h_sql, sprintf('update tasks set spawning_coordinate="%s",notes="pass agg" where id=%d;', excoord,stashed_child_task_id(chid)));
%                         rtn = mysql(h_sql, sprintf('update tasks set spawning_coordinate="%s" where id=%d;', excoord,stashed_child_task_id(chid)));
                ...
                elseif any( (dup_seg_list{1}(:)~=test_cell) .* (dup_seg_list{6}(:)~=1) .* (dup_seg_list{6}~=4) )
                    ...
                    write_log(sprintf('       %d (%s) spawned',last_task_id,drct_str(drct)), 1);
                    
                    count_new = count_new+1;
                    count_all = count_all+1;
                    new_ids = [new_ids last_task_id];
                    
                    %modify by jwgim 180910 (notes field update)
                    rtn = mysql(h_sql, sprintf('update tasks set notes="pass agg" where id=%d;', last_task_id));
                    ...
                    for ii=1:numel(dup_seg_list{2})
                        ...
                        write_log(sprintf('         %d (cell_id:%d) duplication',dup_seg_list{3}(ii),dup_seg_list{1}(ii)), 1);
                    end
