...
function [branch_path_sub, branch_dbf, branch_terminal,iteration_count]= get_cf_skeleton(volume, target_segment_omni_id, p_scale, p_const, close_or_not, largest_cc_only, draw_figure)

    addpath ./subfunctions_for_cf_skeletonization/
...
%    volume = h5read(target_volume_h5,'/main');
    ...
    [volume_new,offset_new] = cut_volume_to_fit_target_segment (volume, target_segment_omni_id, largest_cc_only);
    fprintf('Done.\n');
%%
    iteration_count = 0; 
    if close_or_not
...
%%
    if draw_figure
    figure; hold on; cellfun(@(x,y) scatter3(x(:,1),x(:,2),x(:,3),y.*30),branch_path_sub,branch_dbf); set(gca, 'DataAspectRatio', [1 1 1]);
    end

end

function [volume_new, offset_new] = cut_volume_to_fit_target_segment (volume, target_segment_omni_id, largest_cc_only)

    volume_bool = volume == target_segment_omni_id;

    if largest_cc_only
        volume_bool = leave_the_largest_cc_only(volume_bool);
    end
    ...
    volume_new(2:end-1,2:end-1,2:end-1) = volume_bool (min_x:max_x,min_y:max_y,min_z:max_z);
