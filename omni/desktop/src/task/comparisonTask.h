...
#include "utility/omTimer.hpp"  //jwgim
...
class ComparisonTask : virtual public Task {
 public:
  ...
  ComparisonTask(uint64_t id, uint64_t cellId, const std::string& path,
                 std::vector<SegGroup>&& namedGroups);
  ComparisonTask(om::network::SqlQuery::SqlResultset rsl);  //jwgim
  ...
  virtual uint64_t GroupID() const { return groupId_; }
