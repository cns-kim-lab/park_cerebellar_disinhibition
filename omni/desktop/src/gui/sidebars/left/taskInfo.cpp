...
TaskInfoWidget::TaskInfoWidget(QWidget* parent)
  ...
  //jwgim add
  QLabel* coordinateLabel = new QLabel(tr("Task from "), this); 
  coordinateLabel_ = new QLabel(this);
  layout->addRow(coordinateLabel, coordinateLabel_);
...
void TaskInfoWidget::updateInfo() {
  ...
  coordinateLabel_->setText(task ? QString::fromStdString(task->Taskfrom()) : tr(""));
...
void TaskInfoWidget::taskSelectorButtonPressed() {
  if( !TaskManager::AttemptSkipTask() ) //jwgim
    return;
  QMessageBox box(
...
void TaskInfoWidget::doneButtonPressed() {
  ...
  if( !TaskManager::SubmitTask(false) ) //jwgim
    return;
  OmActions::Save();
  OmAppState::OpenTaskSelector();
}
