...
void InspectorWidget::setRowFlagsAndCheckState(QTreeWidgetItem* row,
...
void InspectorWidget::populateDataSrcListWidget() {
  ...
  FOR_EACH(iter, validChanIDs) {
    ...
    row->setText(ID_COL, QString("%1").arg(cdw.id()));
  ...
  FOR_EACH(iter, validSegIDs) {
    ...
    row->setText(ID_COL, QString("%1").arg(sdw.id()));
...
void InspectorWidget::populateFilterListWidget(ChannelDataWrapper cdw) {
  ...
  ElementListBox::SetTitle(QString("Channel %1").arg(cdw.id()));
  ...
  FOR_EACH(iter, filters) {
    ...
    FilterDataWrapper filter(cdw.id(), filterPtr->GetID());
...
void InspectorWidget::addSegmentationToSplitter(SegmentationDataWrapper sdw) {
  SegmentationInspector* w =
      new SegmentationInspector(this, inspectorProperties_, sdw);
...
void InspectorWidget::addToSplitterDataElementFilter(QTreeWidgetItem* current) {
  ...
  inspectorProperties_->SetOrReplaceWidget(
      filObjectInspectorWidget_,
...
void InspectorWidget::nameEditChanged() {
  ...
  /*
    ...
    Qt::UserRole);
      ...
      proxyModel->setData(view->currentIndex(),
    QVariant(channelInspectorWidget_->nameEdit->text()), Qt::EditRole);
      else if(item_type == SEGMENTATION)
      proxyModel->setData(view->currentIndex(),
    QVariant(segmentationInspectorWidget->nameEdit->text()), Qt::EditRole);
      else if(item_type == SEGMENT)
      proxyModel->setData(view->currentIndex(),
    QVariant(segmentInspectorWidget->nameEdit->text()), Qt::EditRole);
      else if(FILTER == item_type)
      proxyModel->setData(view->currentIndex(),
    QVariant(filObjectInspectorWidget_->nameEdit->text()), Qt::EditRole);
    */
}
...
QWidget* InspectorWidget::setupDataSrcList() {
  ...
  if (t) {
    ...
    tabbed->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Preferred);  //jwgim 170912
    return tabbed;
  } else {
...
void InspectorWidget::showDataSrcContextMenu(const QPoint& menuPoint) {
  ...
  om::connect(makeDataSrcContextMenu(dataSrcListWidget_),
...
void InspectorWidget::showChannelContextMenu() {
  om::connect(makeContextMenuBase(dataSrcListWidget_),
              SIGNAL(triggered(QAction*)), this,
...
void InspectorWidget::showSegmentationContextMenu() {
  om::connect(makeSegmentationContextMenu(dataSrcListWidget_),
              SIGNAL(triggered(QAction*)), this,
...
void InspectorWidget::selectChannelView(QAction* act) {
  ...
  if (propAct_ == act) {
    ...
    triggerChannelView(cdw.id(), getViewType(act));
  }
}
...
void InspectorWidget::selectSegmentationView(QAction* act) {
  ...
  if (propAct_ == act) {
    ...
    triggerSegmentationView(sdw.id(), getViewType(act));
  }
}

void InspectorWidget::addChannelToSplitter(ChannelDataWrapper cdw) {
  channelInspectorWidget_ =
      new ChannelInspector(this, inspectorProperties_, cdw);
...
void InspectorWidget::deleteSegmentation(SegmentationDataWrapper sdw) {
  ...
  confirmDelete.setInformativeText(
      "Are you certain you want to delete this volume?");
  ...
  const om::common::ID segmentationID = sdw.id();
...
void InspectorWidget::deleteChannel(ChannelDataWrapper cdw) {
  ...
  if (confirmDelete.Ask()) {
    inspectorProperties_->CloseDialog();
    ElementListBox::Reset();
    mainWindow_->cleanViewsOnVolumeChange(om::common::CHANNEL, cdw.id());
    cdw.Remove();
    populateDataSrcListWidget();
  }
}
