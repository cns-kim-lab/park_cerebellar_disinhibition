 ...
 private:
  ...
  inline void setSegmentSelectedBatch(const common::SegID segID,
    ...
    if(seg){
      ...
      std::cout << "\tjeson msg, segID " << rootID << " in selection" << std::endl;
#else
  if (isSelected) {
        doSelectedSetInsert(rootID, addToRecentList);
  } else {
...
#else
    // 07.30 - jeson, exception
    if(seg != nullptr) lists_.TouchRecent(seg);    
    else {
      selected_.erase(segID);
      std::cout << "\tjeson msg: erase idx: " << segID << std::endl;
