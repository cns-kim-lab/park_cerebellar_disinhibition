...
class OmSegmentListsImpl {
 ...
 private:
  ...
  void doBuildInitialSegmentList() {
    ...
    uint32_t numSegs = 0;
    for (auto segId = 1; segId <= meta_.maxSegments(); segId++) {
  ...
  void doUpdateSizeListsFromJoin(OmSegment* root, OmSegment* child) {
    ...
    #if 1
      // 08.01 - adding exception by jeson
      if( rootInfo.sizeIncludingChildren > 0 && childInfo.sizeIncludingChildren > 0 ){
