...
%     kaffe_path = '/volume_1/research/jwgim/cnn_pkg/kaffe/python/'; %for old unet
%    kaffe_path = '/data/research/jwgim/cnn_pkg/kaffe_new_cfg/python/'; % for RS unet
    kaffe_path = '/ldata/research/cjpark147/kaffe/python/'; % for syn detection
...
%    fwdsc_path = [kaffe_path 'infer.py'];   %for RS unet
    ...
    % kimworkstation02 localhost computing node:
    gpu_id = 0;
    host_id = 2;
    fid = fopen(fwdscript_path, 'w');   %for sv102, sv103
    fprintf(fid, '#! /bin/bash\n');
    fprintf(fid, 'export PATH=$PATH:/usr/local/cuda/bin\n');
    fprintf(fid, 'export LD_LIBRARY_PATH=/usr/local/cuda/lib64\n');
    fprintf(fid, 'export PATH=$PATH:/ldata/caffe_opencl/caffe/build/tools:/ldata/caffe_opencl/caffe/python\n');   %            for old unet
    fprintf(fid, 'export PYTHONPATH=/ldata/caffe_opencl/caffe/python:/ldata/research/cjpark147/kaffe:/ldata/research/cjpark147/kaffe/layers\n');
...
%            for old unet
            ...
            fclose(fid);
        case {104,105}
            fid = fopen(fwdscript_path, 'w');   %for sv104, sv105
            fprintf(fid, '#! /bin/bash\n');
            fprintf(fid, 'export PATH=$PATH:/usr/local/cuda/bin\n');
            fprintf(fid, 'export LD_LIBRARY_PATH=/usr/local/cuda/lib64\n');
            fprintf(fid, 'export PATH=$PATH:/usr/local/cuda/bin:/usr/local/caffe/caffe/build/tools:/usr/local/caffe/caffe/python\n');
%            for old unet
             fprintf(fid, 'export export PYTHONPATH=:/usr/local/caffe/caffe/python:/data/research/jwgim/cnn_pkg/kaffe:/data/research/jwgim/cnn_pkg/kaffe/layers\n');
            %fprintf(fid, 'export PYTHONPATH=/usr/local/caffe/caffe/python:/data/research/jwgim/cnn_pkg/kaffe_new_cfg:/data/research/jwgim/cnn_pkg/kaffe_new_cfg/layers\n');            
            %for RS unet
            fprintf(fid, ['python ' fwdsc_path ' ' num2str(gpu_id) ' ' fwdcfg_path '>' fwdlog_path ' 2>&1 &disown\n']);
            fclose(fid);            
    end
   %}
    ...
    [~,~] = system(fwdscript_path);
    
    %{ 
    % KBRI copmuting nodes
