% To classify given interfaces as synaptic or non-synaptic. 

%  SYNAPSE PREDICTION

% Requires: SC-Net output, Vesicle cloud segmentation, interface volume, neuron segmentation, interface_segpair.mat, SVM classifier}
% Output = list of synapse info written to text file at /data/lrrtm3_wt_syn/synapse_info.txt.

%  Computes two feature values for every interface using cleft-net output.
%  Using the two features, classifies interfaces using trained SVM.
%  Reduces false negative error by regaining lost high-prob interfaces.  
%  Reduces false positive error by matching vesicle segments 

% testvolume

%{
assembly_size = [992,992,248];
cleftprob_path = '/data/lrrtm3_wt_syn/assembly/assembly_cleft_prob_test.h5';
vesicle_path = '/data/lrrtm3_wt_syn/assembly/vesicle_segmentation_0906_1500_05_test.h5';
intf_path = '/data/lrrtm3_wt_syn/assembly/assembly_interface_relevant_test.h5';
seg_path = '/data/research/share/synapseDetection/testvolume_segmentation.h5';
...
load('/data/lrrtm3_wt_syn/assembly/interface_translator_test.mat');
%}
...
fid = fopen('/data/lrrtm3_wt_syn/synapse_det_info_raw_210503_theta_-1_try.txt','w');
fid2 = fopen('/data/lrrtm3_wt_syn/interface_info_210503_-1_try.txt','w');
...
lowerPrctile = 85;
highProb = 0.9;
svm_theta = -0.2;     % 0.95 precision & 0.95 recall
...
parfor ridx=startpoint:halfpoint
...
save('/data/lrrtm3_wt_syn/mat_data/synapse_info_tbl_test.mat', 'synapse_info_tbl');
...
save('/data/lrrtm3_wt_syn/mat_data/predictions_test.mat', 'predictions');
...
parfor ridx=(halfpoint+1):endpoint
...
fprintf('\n *************************************************** ' );
fprintf('%s %d\n', 'Classifying large interfaces  N = ', numel(intf_ids_beyond_limit));
fprintf('\n *************************************************** \n' );

%% classify interfaces requiring vol size > load_limit
...
for ridx=1:numel(intf_ids_beyond_limit)
    ...
    stp = max([1,1,1], [bbox_fixed(intf_ids_beyond_limit(ridx),1:2) - vc_dist,  bbox_fixed(intf_ids_beyond_limit(ridx),3) - ceil(vc_dist/z_scaling) ]);   % bbox x,y,z
    enp = min(vol_size, stp - [1,1,1] + [bbox_fixed(intf_ids_beyond_limit(ridx),4:5) + vc_dist *2, bbox_fixed(intf_ids_beyond_limit(ridx),6) + ceil(vc_dist/z_scaling)*2]);
    num_elem = enp - stp + 1;
    cleft_net_output = h5read(cleftprob_path, '/main', stp, num_elem);
    intf_vol = h5read(intf_path, '/main', stp, num_elem);
    cells_segment_vol = h5read(seg_path, '/main', stp, num_elem);
    ...
    cprob = cleft_net_output(intf_idx);
    clear cleft_net_output;
   % compute intf sizes
    intf_size = numel(intf_idx);
    intf_vol_iso_mip0 = imresize3(intf_vol,'Scale', [1,1,4],'Method', 'nearest');
    intf_size_iso_mip0 = round(numel(find(intf_vol_iso_mip0 == intf_ids_within_limit(ridx)))/2);        
    intf_size_info(ridx2,1) = intf_size_iso_mip0;
    ...
    if (intf_size > intf_size_th)
        ...
        [~,score] = predict(SVMModel, [prct, interprct]);
        label = double(score(:,2) > svm_theta) + 1;
        predictions(ridx2,1) = label;
        bbox_info(ridx2,:) = [stp, num_elem];
    end
    
    % very large interfaces
    if sum(cprob > highProb, 'all') > (intf_size_th * 3)
        predictions(ridx2,1) = 2;
        bbox_info(ridx2,:) = [stp, num_elem];
    end
    ...
    if predictions(ridx2,1) == 2
        ...
        if ~isempty(vc_seg_match)
            ...
            bw_vc = imerode(bw_vc, se);
            vc_surf = (uint32(vesicle_seg_vol) .* uint32(~bw_vc));
            clear bw_vc vesicle_seg_vol;
            [ix,iy,iz] = ind2sub(size(intf_vol), find(intf_vol == intf_ids_beyond_limit(ridx)));
            clear intf_vol
            ...
            if ismember(cell_type1, [2,3,4])
                ...
                if numel(vc_of_seg) > 0
                    ...
                    contact_info(ridx2,:) = contact_loc + stp - 1;
                end
            end
            ...
            if dist > vc_dist && dist2 > vc_dist                        % if no vesicle exists within vc_dist from intf
                predictions(ridx2,1) = 1;                                % predict value 1
                
            elseif dist <= vc_dist && dist2 <= vc_dist                    % if both seg have vc within vc_dist
             
                pre_post_ambiguity(ridx2,1) = 1;                
                fprintf(['intf id ' ,num2str(intf_ids_beyond_limit(ridx)), ': detected vesicle in both cells. \n']);
                
                if contact_size >= contact_size2        % if vc in seg1 has larger contacts
                    % do nothing
                elseif contact_size < contact_size2    % if vc in seg2 has larger contacts
                    seg_id_info(ridx2,:) = [seg_id2, seg_id1];             % swap seg id positions
                    cell_type_info(ridx2,:) = [cell_type2, cell_type1];    % swap cell type positions
                    contact_info(ridx2,:) = contact_loc2 + stp - 1;
                end
    ...
    fprintf('\n ***  DONE  ***\n');
    fprintf('\n **************************************************************** ' );
    fprintf(' saving prediction info... \n');
    

    
%%  save beyond load_limit synapse information to file

synapse_info_tbl = [intf_id_info, seg_id_info, cell_type_info, intf_size_info, contact_info, bbox_info];
save('/data/lrrtm3_wt_syn/mat_data/synapse_info_tbl_test.mat', 'synapse_info_tbl');

for i = (numel(intf_ids_within_limit)+1):numel(predictions)
    val1 = cell_type_info(i,1);
    val2 = cell_type_info(i,2);
    fprintf(fid2, '%d,%d,%d,%s,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n', intf_id_info(i,1), seg_id_info(i,:), celltype_map{val1}, celltype_map{val2}, ...
            intf_size_info(i,1), contact_info(i,:), bbox_info(i,:));   
